...
def test_example(page: Page) -> None:
    ...
    page.wait_for_timeout(2000)  # Wait for any animations/transitions
    ...
    # Wait for DELETE button in confirmation dialog to be ready
    # Use test-id for more specific targeting to avoid confusion with previous Delete button
    delete_confirm_button = page.get_by_test_id("customer-organization-astana-tegal-gundul-a20b2010-manage-edit-interment-ng-component-button-mat-focus-indicator")
    delete_confirm_button.wait_for(state='visible', timeout=10000)
    expect(delete_confirm_button).to_be_enabled()
    delete_confirm_button.scroll_into_view_if_needed()
    page.wait_for_timeout(1000)  # Wait for dialog animation
    delete_confirm_button.click(delay=200)
